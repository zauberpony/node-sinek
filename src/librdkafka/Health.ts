...
export class Check {
     ...
     */
    constructor(public status = STATES.HEALTHY, message: string | string[] = MESSAGES.HEALTHY) {
...
class Health {
    protected config;
    private STATES: { DIS_ANALYTICS: number; NO_ANALYTICS: number; UNKNOWN: number; UNCONNECTED: number; HEALTHY: number; RISK: number; WARNING: number; CRITICAL: number };
    private MESSAGES: { DIS_ANALYTICS: string; NO_ANALYTICS: string; UNKNOWN: string; UNCONNECTED: string; HEALTHY: string };

    /**
     * creates a new instance
     * @param {NConsumer|NProducer} client
     */
    constructor(protected client, config) {
